    ...
    internal static class DataBlocksGlobal
    {
        ...
        public static void GetDatablockElements(PlcBlockGroup blockGroup)
        {
            ...
            foreach (PlcBlock block in blockGroup.Blocks)
            {
                ...
                if (block.ProgrammingLanguage == ProgrammingLanguage.DB && OpcUaGenerationContext.Current.GlobalDBsAccessLevel != 0)
                {
                    NewDataBlockElement(block, parentFolder, false);
                }
                //Only process failsafe Datablocks
                if (block.ProgrammingLanguage == ProgrammingLanguage.F_DB && OpcUaGenerationContext.Current.SafetyGlobalDBsAccessLevel != 0)
                {
        ...
        {
            if (OpcUaGenerationContext.Current.KeepFolderStructure)
            {
                ...
                XElement uaObjectElement =
                    new XElement(OpcUaGenerationContext.Current.RootNameSpace + "UAObject",
                        new XAttribute("NodeId", $"ns=2;s={fullPathName}"),
                        new XAttribute("BrowseName", $"2:{folderName}"),
                        new XElement(OpcUaGenerationContext.Current.RootNameSpace + "DisplayName", folderName),
                        new XElement(OpcUaGenerationContext.Current.RootNameSpace + "References",
                            new XElement(OpcUaGenerationContext.Current.RootNameSpace + "Reference", $"ns=2;s={parentFolder}",
                                new XAttribute("ReferenceType", "HasComponent"),
                                new XAttribute("IsForward", "false")
                            ),
                            new XElement(OpcUaGenerationContext.Current.RootNameSpace + "Reference", "i=61",
        ...
        private static void NewDataBlockElement(PlcBlock block, string parentFolder, bool isSafety)
        {
            ...
            {
                OpcUaGenerationLog.Progress = $@"Adding ""Global"" DBs to the server interface... Count: {_numberOfProcessedDBs}";
            }
            ...
            {
                throw new InvalidOperationException($"Data block '{block.Name}' is inconsistent. Compile the PLC before generating the OPC UA interface.");
            }
            ;

            // Export the data block as a .txt file
            string filePath = Path.Combine(OpcUaGenerationContext.Current.WorkingDirectory, Guid.NewGuid().ToString("N") + ".xml");
            ...
            {
                throw new InvalidOperationException($"Could not export global DB '{block.Name}'.", ex);
            }
            #endregion
            ...
            {
                throw new InvalidOperationException($"Could not parse exported global DB '{block.Name}'.", ex);
            }
            #endregion
            ...
            if (OpcUaGenerationContext.Current.IsSoftwareUnit)
            {
                OpcUaGenerationContext.Current.SoftwareUnitNamespace = attributeList.Element("Namespace").Value;
            }
            #endregion
            ...
            XElement uaObjectElement =
                new XElement(OpcUaGenerationContext.Current.RootNameSpace + "UAObject",
                    ...
                    new XElement(OpcUaGenerationContext.Current.RootNameSpace + "DisplayName", blockName),
                    new XElement(OpcUaGenerationContext.Current.RootNameSpace + "References",
                        new XElement(OpcUaGenerationContext.Current.RootNameSpace + "Reference", $"ns=2;s={parentFolder}",
                            new XAttribute("ReferenceType", "HasComponent"),
                            new XAttribute("IsForward", "false")
                        ),
                        new XElement(OpcUaGenerationContext.Current.RootNameSpace + "Reference", "i=58",
                            new XAttribute("ReferenceType", "HasTypeDefinition")
                        )
                    )
                );
            ...
            if (currentElementsCount == newElementsCount && OpcUaGenerationContext.Current.KeepEmptyDBs == false) // No new nodes were added to the server interface
            {
                BuildDataBlockElements.XElementDataBlocks.Remove(uaObjectElement);
                OpcUaGenerationLog.Publish($@"EMPTY DB: The Global DB ""{blockName}"" has not been added to the server interface as it does not contain any variables accessible via OPC UA.");
            }
            #endregion
        }
    }
